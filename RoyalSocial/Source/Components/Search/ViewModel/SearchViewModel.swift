...
class SearchViewModel: ObservableObject {
    ...
    private func fetchUsers(_ snapshot: QuerySnapshot?) async throws {
        ...
        guard let currentUserData = try? await UserService
            .fetchUser(withUid: currentUser.uid) else { return }
//        let _ = currentUserData.blockedUsers ?? [:]

        guard let documents = snapshot?.documents else { return }

        // Create a temporary array to store the fetched users
        var tempUsers: [User] = []
        ...
        try await withThrowingTaskGroup(of: User.self) { taskGroup in
            ...
            for try await user in taskGroup {
                // Only append the user if they are not blocked
                let isBlocked = try await BlockingService.isUserBlocked(currentUid: currentUser.uid, otherUid: user.uid ?? "")
        ...
        // Assign the fetched users to the class/instance variable
